...
Well, $$i(W;M)\preceq \Delta(P(WM),P(W)P(M)) \preceq I(W;M)$$ is done. There is only one relation left $$\frac{I(W;M)}{n}\preceq i(W;M)$$.

#(2): $$
\begin{split}\frac{I(W;M)}{n}=E(\frac{i(W;M)}{n})=&\underbrace{E(\frac{i(W;M)}{n}\mathds{1}\{i(W;M)\leq -\epsilon\})}_{<0}\\
...
#  &\frac{\epsilon}{n}+\frac{\log_2|M|}{n}P(i(W;M)>\epsilon)\end{split}
#$$
#
#What does security mean eventually
#
#$$ \begin{itemize}
#   \item Совершенная секретность $I(W;M)=0$
#   \item Сильная секретность $\lim_{n\rightarrow \infty} I(W;M)=0$
#    \item Слабая секретность $\lim_{n\rightarrow \infty} \frac{I(W;M)}{n}=0$
#   \end{itemize}
#$$
#
#\begin{itemize}
#  \item $I(W;M)=0 \;\;\;\Rightarrow \; |K|\geq |M|$
#  \item $\lim_{n\rightarrow \infty}I(W;M)=0\;\;\;\Rightarrow \;p_{err}=1-2^{-n}$
#  \item $\lim_{n\rightarrow \infty}\frac{I(W;M)}{n}=0\;\;\;\Rightarrow \; p_{err}=1-o(1)$
#    \end{itemize}
#  \begin{lem}
